...
function pluginValidate(plugin, jobNames){

    for(var i=0; i<jobNames.length; i++){
        if(plugin.substring(plugin.lastIndexOf('\\')+1, plugin.indexOf('Plugin.jar'))+'Job'==jobNames[i].innerHTML){
            return false;
        }
    }
    return true;
...
function generateJobName(plugin){

     var startIndex = plugin.lastIndexOf('\\')+1;
    var endIndex = plugin.lastIndexOf('Plugin.jar');

   return  plugin.substring(startIndex, endIndex)+'Job';
